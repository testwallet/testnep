...
pkgdatadir = $(datadir)/Nepton
pkgincludedir = $(includedir)/Nepton
pkglibdir = $(libdir)/Nepton
pkglibexecdir = $(libexecdir)/Nepton
...
DIST_COMMON = $(srcdir)/Makefile.in $(srcdir)/Makefile.am \
	...
	$(top_srcdir)/src/config/Nepton-config.h.in \
...
CONFIG_HEADER = $(top_builddir)/src/config/Nepton-config.h
...
ACLOCAL = ${SHELL} /root/zNepton/build-aux/missing aclocal-1.14
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/zNepton/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/zNepton/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/zNepton/build-aux/missing automake-1.14
AWK = mawk
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = Nepton-cli
BITCOIN_DAEMON_NAME = Neptond
BITCOIN_GUI_NAME = Nepton-qt
BITCOIN_TX_NAME = Nepton-tx
...
MAINT = 
MAKEINFO = ${SHELL} /root/zNepton/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = Nepton
PACKAGE_BUGREPORT = www.Nepton.org
PACKAGE_NAME = Nepton Core
PACKAGE_STRING = Nepton Core 1.0.0
PACKAGE_TARNAME = Nepton
PACKAGE_URL = 
...
ZMQ_CFLAGS =  
ZMQ_LIBS = -lzmq  
abs_builddir = /root/zNepton
abs_srcdir = /root/zNepton
abs_top_builddir = /root/zNepton
abs_top_srcdir = /root/zNepton
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/zNepton/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_Nepton_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_Nepton_coverage.info test_Nepton.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/NEP-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/Nepton-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/Nepton-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/Nepton-config.h
$(top_srcdir)/src/config/Nepton-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/Nepton-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/NEP-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/NEP-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/NEP-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_Nepton.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_Nepton -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_Nepton_filtered.info: test_Nepton.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_Nepton_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_Nepton_coverage.info: baseline_filtered_combined.info test_Nepton_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_Nepton_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_Nepton_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_Nepton_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_Nepton.coverage/.dirstamp:  test_Nepton_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_Nepton.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_Nepton.coverage/ total.coverage/ $(OSX_APP)
