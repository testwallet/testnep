...
* "push" notifications (ZMQ and `-neptonfastnotify` cmd-line/config option);
* RPC commands.
...
This mechanism has been integrated into Bitcore-Node-Nepton which allows for notification to be broadcast through Insight API in one of two ways:
* WebSocket: 
* API:  
...
By default, the Nepton Core daemon will launch using the following constant:

```
static const int DEFAULT_NEPTONFAST_DEPTH = 5;
```

This value can be overridden by passing the following argument to the Nepton Core daemon:
