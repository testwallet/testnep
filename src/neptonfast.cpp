...
//step 2.) Top NEPTONFAST_SIGNATURES_TOTAL masternodes, open connect to top 1 masternode.
//         Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 masternode, waits for NEPTONFAST_SIGNATURES_REQUIRED messages. Upon success, sends "txlock'

void ProcessMessageNeptonFAST(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if (fLiteMode) return; //disable all obfuscation/masternode related functionality
    if (!IsSporkActive(SPORK_2_NEPTONFAST)) return;
    if (!masternodeSync.IsBlockchainSynced()) return;

    if (strCommand == "ix") {
        //LogPrintf("ProcessMessageNeptonFAST::ix\n");
        ...
        BOOST_FOREACH (const CTxOut o, tx.vout) {
            ...
            if (!o.scriptPubKey.IsNormalPaymentScript() && !o.scriptPubKey.IsUnspendable()) {
                LogPrintf("ProcessMessageNeptonFAST::ix - Invalid Script %s\n", tx.ToString().c_str());
                return;
            }
        }
        ...
        if (fAccepted) {
            ...
            LogPrintf("ProcessMessageNeptonFAST::ix - Transaction Lock Request: %s %s : accepted %s\n",
            ...
            LogPrintf("ProcessMessageNeptonFAST::ix - Transaction Lock Request: %s %s : rejected %s\n",
                pfrom->addr.ToString().c_str(), pfrom->cleanSubVer.c_str(),
                tx.GetHash().ToString().c_str());
            ...
            if (i != mapTxLocks.end()) {
                ...
                if ((*i).second.CountSignatures() >= NEPTONFAST_SIGNATURES_REQUIRED) {
                    if (!CheckForConflictingLocks(tx)) {
                        LogPrintf("ProcessMessageNeptonFAST::ix - Found Existing Complete IX Lock\n");
            ...
            */
            if (!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)) {
                ...
                if (mapUnknownVotes[ctx.vinMasternode.prevout.hash] > GetTime() &&
                    ...
                    LogPrintf("ProcessMessageNeptonFAST::ix - masternode is spamming transaction votes: %s %s\n",
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = mnodeman.GetMasternodeRank(activeMasternode.vin, nBlockHeight, MIN_NEPTONFAST_PROTO_VERSION);
...
{
    int n = mnodeman.GetMasternodeRank(ctx.vinMasternode, ctx.nBlockHeight, MIN_NEPTONFAST_PROTO_VERSION);
...
bool CTransactionLock::SignaturesValid()
{
    BOOST_FOREACH (CConsensusVote vote, vecConsensusVotes) {
        int n = mnodeman.GetMasternodeRank(vote.vinMasternode, vote.nBlockHeight, MIN_NEPTONFAST_PROTO_VERSION);
