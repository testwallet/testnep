...
#if !defined(HAVE_MSG_NEPIGNAL) && !defined(MSG_NEPIGNAL)
#define MSG_NEPIGNAL 0
#endif
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Nepton users should be well used to small numbers with many decimal places by now :)
...
void SocketSendData(CNode* pnode)
{
    ...
    while (it != pnode->vSendMsg.end()) {
        ...
        int nBytes = send(pnode->hSocket, &data[pnode->nSendOffset], data.size() - pnode->nSendOffset, MSG_NEPIGNAL | MSG_DONTWAIT);
        if (nBytes > 0) {
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "Nepton " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    setsockopt(hListenSocket, SOL_SOCKET, SO_NEPIGPIPE, (void*)&nOne, sizeof(int));
#endif
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Nepton Core is probably already running."), addrBind.ToString());
        else
