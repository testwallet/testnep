...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    if (nQuantity > 0) {
        ...
        // IX Fee
        if (coinControl->useNeptonFAST) nPayFee = max(nPayFee, CENT);
        // Allow free?
    ...
    int nDisplayUnit = BitcoinUnits::NEP;
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) coins, mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show Nepton address. In tree mode, address is not shown again for direct wallet address outputs
