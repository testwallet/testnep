...
#include "zNeptoncontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zNEP ought to be enough for anybody." - Bill Gates, 2017
    ui->zNEPpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzNEPSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzNEP->setEnabled(false);
        ui->pushButtonMintzNEP->setToolTip(tr("zNEP is currently disabled due to maintenance."));

        ui->pushButtonSpendzNEP->setEnabled(false);
        ui->pushButtonSpendzNEP->setToolTip(tr("zNEP is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zNEPpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzNEP_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNEP is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zNEP...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zNEP in ") +
...
void PrivacyDialog::on_pushButtonSpendzNEP_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNEP is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zNEP
        sendzNEP();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zNEP
    sendzNEP();
}

void PrivacyDialog::on_pushButtonZNeptonControl_clicked()
{
    ZNeptonControlDialog* zNeptonControl = new ZNeptonControlDialog(this);
    zNeptonControl->setModel(walletModel);
    zNeptonControl->exec();
}

void PrivacyDialog::setZNeptonControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzNeptonSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzNEP()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Nepton Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zNEPpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zNEP is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " NEP </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zNEP</b>";
    ...
    // use mints from zNepton selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZNeptonControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZNeptonControlDialog::GetSelectedMints();
    }
    ...
    // Clear zNepton selector in case it was used
    ZNeptonControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zNepton Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Nepton, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zNepton Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zNEP </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zNEP "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zNEP "));
    ui->labelzNEPAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzNEPSyncStatus->setVisible(fShow);
}
