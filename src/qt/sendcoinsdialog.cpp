                                                    ...
                                                    fFeeMinimized(true)
{
    ...
    if (!settings.contains("bUseNeptonFAST"))
        settings.setValue("bUseNeptonFAST", false);

    bool useNeptonFAST = settings.value("bUseNeptonFAST").toBool();
    if (fLiteMode) {
        ui->checkNeptonFAST->setVisible(false);
        CoinControlDialog::coinControl->useObfuScation = false;
        CoinControlDialog::coinControl->useNeptonFAST = false;
    } else {
        ui->checkNeptonFAST->setChecked(useNeptonFAST);
        CoinControlDialog::coinControl->useNeptonFAST = useNeptonFAST;
    }

    connect(ui->checkNeptonFAST, SIGNAL(stateChanged(int)), this, SLOT(updateNeptonFAST()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (ui->checkNeptonFAST->isChecked()) {
        recipients[0].useNeptonFAST = true;
        strFunds += " ";
        strFunds += tr("and SwiftX");
    } else {
        recipients[0].useNeptonFAST = false;
    }
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        ...
        // generate moNeptonpace address string
        QString address = "<span style='font-family: moNeptonpace;'>" + rcp.address;
        address.append("</span>");
...
void SendCoinsDialog::updateNeptonFAST()
{
    QSettings settings;
    settings.setValue("bUseNeptonFAST", ui->checkNeptonFAST->isChecked());
    CoinControlDialog::coinControl->useNeptonFAST = ui->checkNeptonFAST->isChecked();
    coinControlUpdateLabels();
}
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Nepton address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
