...
#include "ui_zNeptoncontroldialog.h"
...
std::list<std::string> ZNeptonControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZNeptonControlDialog::listMints;

ZNeptonControlDialog::ZNeptonControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZNeptonControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZNeptonControlDialog::~ZNeptonControlDialog()
{
    delete ui;
}

void ZNeptonControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNeptonControlDialog::updateList()
{
...
void ZNeptonControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNeptonControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZNepton_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNeptonControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZNeptonControlDialog::GetSelectedMints()
{
...
void ZNeptonControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
